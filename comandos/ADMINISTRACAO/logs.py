...
class Logs(commands.Cog):
    ...
    def _config(self, guild_id):
        return CONFIG.find_one({"guild_id": guild_id}) or {}
    ...
    async def logs(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description="Configure o canal que receberá os registros administrativos do servidor.",
    ...
    async def logs_channel(self, ctx, channel: discord.TextChannel):
        ...
        await ctx.send(embed=discord.Embed(
            ...
            description=f"Os registros serão enviados para {channel.mention}.",
    ...
    async def logs_disable(self, ctx):
        ...
        await ctx.send(embed=discord.Embed(
            ...
            description="Nenhum novo registro será enviado pelo sistema de logs.",
    ...
    async def on_member_update(self, before, after):
        if before.roles != after.roles:
            ...
            if added or removed:
                embed = discord.Embed(
                    title="🛡️ Cargos alterados",
                    description=f"Alteração de cargos em {after.mention}.",
    ...
    async def on_guild_channel_create(self, channel):
        embed = discord.Embed(
            title="➕ Canal criado",
            description=f"{channel.mention} foi criado.",
            color=discord.Color.green(),
            timestamp=discord.utils.utcnow()
        )
    ...
    async def on_guild_channel_delete(self, channel):
        embed = discord.Embed(
            title="➖ Canal apagado",
            description=f"`{channel.name}` foi apagado.",
            color=discord.Color.red(),
            timestamp=discord.utils.utcnow()
        )
        embed.add_field(name="Tipo", value=str(channel.type), inline=True)
