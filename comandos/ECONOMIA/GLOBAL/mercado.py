...
CATEGORIAS_POR_MERCADO = {
    "taverna": {"comida", "bebida", "hospedagem"},
    "loja": {"item", "equipamento", "consumivel", "arma", "armadura", "utilidade"},
    "bazar": {"roupa", "vestuario", "acessorio", "tecido"},
}
...
class MercadoEconomico:
    ...
    def categoria_permitida(self, mercado, categoria):
        categoria = str(categoria or "item").lower()
        permitidas = CATEGORIAS_POR_MERCADO.get(mercado.get("tipo"), set())
        return categoria in permitidas
    ...
    def registrar_produto(self, produto_id, nome, preco_base_bronze, categoria, estoque=100, custo_unitario=None):
        self.produtos.update_one(
            ...
            {"$set": {
                ...
                "preco_base_bronze": max(1, float(preco_base_bronze)),
    ...
    def garantir_estoque(self, guild_id, channel_id, produto):
        ...
        if pid not in estoque:
            self.motor.mercados.update_one(
                {"_id": mercado["_id"]},
                {"$set": {f"estoque.{pid}": int(produto.get("estoque_padrao", 100))}}
            )
            mercado = self.motor.mercado_do_canal(guild_id, channel_id)
        return mercado
    ...
    def cotar(self, guild_id, channel_id, produto, quantidade=1):
        ...
        estoque = int(mercado.get("estoque", {}).get(str(produto["produto_id"]), 0))
        if estoque < quantidade:
            return {"erro": "estoque_insuficiente", "estoque": estoque, "mercado": mercado}
    ...
    def comprar(self, guild_id, channel_id, user_id, produto, quantidade=1):
        ...
        if not cotacao or "erro" in cotacao:
            return cotacao
        quantidade = max(1, int(quantidade))
        pid = str(produto["produto_id"])
        mercado = cotacao["mercado"]
        resultado = self.motor.mercados.update_one(
            {"_id": mercado["_id"], f"estoque.{pid}": {"$gte": quantidade}},
            {"$inc": {f"estoque.{pid}": -quantidade,
                       "receita_bronze": cotacao["preco_total"],
        ...
        self.transacoes.insert_one({
            ...
            "user_id": str(user_id), "produto_id": pid,
            "quantidade": quantidade, "valor_bronze": cotacao["preco_total"],
            "tipo": "compra", "criado_em": datetime.now(timezone.utc)
        })
        return cotacao
