...
class Loja(commands.Cog):
    ...
    async def comprar(self, ctx, item_id, quantidade: int = 1):
        ...
        produto = self._produto(item)
        cotacao = self.mercado_economico.cotar(ctx.guild.id, ctx.channel.id, produto, quantidade)
        if 'erro' in cotacao:
            erros = {'categoria_invalida': 'Este produto não pode ser vendido neste estabelecimento.', 'estoque_insuficiente': 'Estoque insuficiente.', 'mercado_nao_configurado': 'Mercado não configurado.'}
            await ctx.send(f"❌ {erros.get(cotacao['erro'], 'Erro na cotação.')}"); return
        ...
        if not db_hunos.remove_hunos(str(ctx.author.id), str(ctx.guild.id), resultado['preco_total']):
            self.motor.mercados.update_one({'_id': resultado['mercado']['_id']}, {'$inc': {f"estoque.{produto['produto_id']}": quantidade}})
            await ctx.send('❌ Erro ao processar o pagamento.'); return
        if not self._adicionar_inventario(str(ctx.author.id), str(ctx.guild.id), item_id, quantidade):
            db_hunos.add_hunos(str(ctx.author.id), str(ctx.guild.id), resultado['preco_total'])
            self.motor.mercados.update_one({'_id': resultado['mercado']['_id']}, {'$inc': {f"estoque.{produto['produto_id']}": quantidade}})
            await ctx.send('❌ Erro ao atualizar inventário. Pagamento revertido.'); return
    ...
    async def categoria(self, ctx, categoria: str):
        mercado = await self._mercado(ctx)
        if not mercado: return
        embed = discord.Embed(title=f'📂 {categoria.capitalize()}', color=discord.Color.gold())
        for item in self.itens[:50]:
            if (item.get('categoria') or item.get('tipo', '')).lower() != categoria.lower(): continue
            produto = self._produto(item)
            if not self.mercado_economico.categoria_permitida(mercado, produto['categoria']): continue
            cotacao = self.mercado_economico.cotar(ctx.guild.id, ctx.channel.id, produto)
            if 'erro' not in cotacao: embed.add_field(name=item['nome'], value=f"`{item['id']}` — {cotacao['preco_unitario']:,} Hunos", inline=False)
        await ctx.send(embed=embed)
