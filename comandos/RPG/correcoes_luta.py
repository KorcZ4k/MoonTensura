...
class CorrecoesLuta(commands.Cog):
    ...
    def _eh_magia_defensiva(self, dados_magia):
        ...
        defesa_base = float(
            dados_magia.get("defesa_base", 0) or 0
        )
    ...
    def _aplicar_correcoes(self):
        ...
        async def usar_magia_corrigida(cog, ctx, dados_magia):
            ...
            mana_base = int(
                float(dados_magia.get("mana_base", 0) or 0)
            )
            mana_atual = int(
                float(usuario.get("mana", 0) or 0)
            )

            if mana_atual < mana_base:
                await ctx.send(
                    f"❌ Mana insuficiente. Necessário: {mana_base}."
                )
                return True
            ...
            defesa_base = int(
                float(dados_magia.get("defesa_base", 0) or 0)
            )
            if defesa_base <= 0:
                defesa_base = int(
                    float(efeito.get("valor", 0) or 0)
                )

            # Esta magia é uma ação defensiva, não um ataque pendente.
            ...
            # Encerra a ação normalmente. O próximo ataque encontrará a
            # defesa mágica ativa no participante protegido.
        ...
        async def resolver_corrigido(cog, ctx):
            ...
            if defensor and defensor.get("magia_defensiva_ativa"):
                bonus = float(
                    defensor.get("defesa_bonus_magica", 0) or 0
                )
                if bonus:
                    defesa_atual = float(
                        defensor.get("defesa", 0) or 0
                    )
            ...
            try:
                ...
                # A barreira protege contra um ataque e então é consumida.
                if defensor and defensor.get("magia_defensiva_ativa"):
                    if bonus:
                        defesa_atual = float(
                            defensor.get("defesa", 0) or 0
        ...
        luta.usar_magia_no_combate = types.MethodType(
            usar_magia_corrigida,
            luta,
        )
        luta._resolver_ataque = types.MethodType(
            resolver_corrigido,
            luta,
        )
    ...
    async def on_ready(self):
        # Também tenta novamente após todas as extensões estarem prontas.
        self._aplicar_correcoes()
